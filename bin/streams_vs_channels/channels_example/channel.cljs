...
(defn process-with-channels
  []
  (let [[fizz-stream buzz-stream fizzbuzz-stream others-stream :as streams] (tools/create-writeable-streams)
    ...
    (let [ch1 (process-channel-3 fizz-chan fizz-stream #(group-string-3 fizz-group %))
      ...
      (async/go
        ...
        (doseq [[stream name counter] (map vector streams ["fizz" "buzz" "fizzbuzz" "others"] counters)]
